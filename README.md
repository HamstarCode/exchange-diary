...
5:00の提出締切後、現在のExchangeに残っている未マッチSubmissionを対象に処理する。

まず性格タイプの相性を利用してマッチングし、それでも残ったSubmissionは条件なしでマッチングする。

```text
① 現在のExchangeの未マッチSubmissionを取得
        ↓
② ユーザーのpersonality_typeを取得
        ↓
③ MATCHING_TYPEから相性のよいタイプを確認
        ↓
④ 相性のよい未マッチSubmissionを探す
        ↓
⑤ 候補がいる？
        ↓ YES
⑥ RoomをINSERT
...
⑧ マッチした2人を対象から除外
        ↓
⑨ 次のSubmissionについて相性マッチング
        ↓
⑩ 相性マッチングできなくなった？
        ↓ YES
⑪ 残ったSubmissionを2人ずつ組み合わせる
        ↓
⑫ RoomをINSERT
        ↓
⑬ 2人のSubmissionにroom_idを設定
...
性格タイプの組み合わせは`MATCHING_TYPE`で定義する。
...
性格タイプによるマッチングでは、相性のよい相手が見つからないSubmissionは一旦残る。

その後、相性マッチングがこれ以上成立しなくなった時点で、残ったSubmissionを条件なしで2人ずつ組み合わせる。

人数が奇数の場合、最後の1人はRoomを作らず未マッチのまま残る。

強制マッチングでは`target_public_user_id`は変更せず、ユーザーが誰を指定していたかの情報は保持する。